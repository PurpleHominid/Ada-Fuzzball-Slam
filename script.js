...
const VP_WIDTH = 920, VP_HEIGHT = 690; //declare variables to hold the viewport size
...
function apply_angularvelocity() {
	for(let i = 0; i < crates.length; i++) {
		Matter.Body.setAngularVelocity( crates[i].body, Math.PI/get_random(3, 20));
	}
};


function apply_force() { //apply the same force to all crates 
	for(let i = 0; i < crates.length; i++) {
...
function level1(replay = false) {
	...
	for(let i = 0; i < MAX_CRATES; i++) { //loop for each instance of a crates
		let top = (-CRATE_HEIGHT * MAX_CRATES ) - 100;
		let offset = (i * CRATE_HEIGHT) * 3;
		crates[i] = new c_crate(700, (top + offset), CRATE_WIDTH, CRATE_HEIGHT, "crate");
	}
...
function paint_assets() {
	for(let i = 0; i < crates.length; i++) { //loop through the crates array and show each
		crates[i].show()	
	}
